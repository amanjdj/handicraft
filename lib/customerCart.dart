import 'dart:async';
import 'package:handicraft/confirmOrderViaCart.dart';
...
class _CustomerCartState extends State<CustomerCart> {
  ...
  Future<void> getCartItems()async{
    ...
    for(int i=0;i<widget.cartCount.length;i++){
      ...
      CartCard cart=CartCard(title: data.data()['title'],price: data.data()['price'],itemID: data.id,available: data.data()['available']);
      cartItems.add(cart);
              ...
              children: [
                ...
                Expanded(
                  child:Container(
                    child: cartItems.length==0?Text("Cart Empty"):ListView.builder(itemCount:cartItems.length,itemBuilder: (_,index){
                      return CartItems(context,cartItems[index].title, cartItems[index].price, cartItems[index].itemID,cartItems[index].available);
                    }),
                  ),
                ),
                ElevatedButton(onPressed:(){
                  void checkOut()async{
                    ...
                    if(flag==0){
                      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Remove out of stock item")));
                      print(flag);
                    }
                    else{
                      print(flag);
                      Navigator.push(context, MaterialPageRoute(builder: (context)=>ConfirmViaCart(cartCount: widget.cartCount,)));
                    }
                  }
                  checkOut();
  ...
  Widget CartItems(BuildContext context,String title,String price,String itemID,String available){
